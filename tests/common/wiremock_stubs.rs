...
/// Registers a wiremock stub for POST /v1/embeddings returning a response with no `usage` field.
/// Used to verify that vector and cost counters are suppressed when usage data is absent.
pub async fn stub_openai_embeddings_no_usage(server: &MockServer, model: &str) {
    let body = fixtures::openai_embeddings_no_usage_response(model);
    let mock = wiremock::Mock::given(method("POST"))
        .and(path(EMBEDDINGS_PATH))
        .respond_with(ResponseTemplate::new(200).set_body_json(body));
    server.register(mock).await;
...
/// Registers a wiremock stub for POST /v1/embeddings returning a 500 error.
pub async fn stub_openai_embeddings_error(server: &MockServer, status: u16) {
    let mock = wiremock::Mock::given(method("POST"))
        .and(path(EMBEDDINGS_PATH))
