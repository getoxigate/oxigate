...
/// Builds an OpenAI-compatible `/v1/embeddings` response with the `usage` field omitted.
/// Used to test the handler's "no usage" branch where cost and vector counters must not fire.
pub fn openai_embeddings_no_usage_response(model: &str) -> serde_json::Value {
    json!({
        "object": "list",
        "data": [{
            "object": "embedding",
