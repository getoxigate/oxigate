...
/// AC1: DeepSeek-V4 Pro chat request is forwarded and cost is tracked.
#[tokio::test]
async fn compat_deepseek_v4_pro_routes_and_tracks_cost() {
    // real URL: https://api.deepseek.com — wiremock substitutes in the test
    assert_compat_cost_tracked("deepseek-v4-pro", "deepseek").await;
}

/// AC2: deepseek-v4-flash cost reflects cache_read_multiplier 0.02 arithmetic.
#[tokio::test]
async fn compat_deepseek_v4_flash_routes_and_tracks_cost() {
    let pg = PgContainer::start().await.expect("pg must start");
    let redis = RedisContainer::start().await.expect("redis must start");

    let mock = wiremock::MockServer::start().await;
    // 100,000 prompt tokens, 50,000 of which are cached; 20,000 completion tokens. Deliberately
    // large: at deepseek-v4-flash's rate (1.4e-7 USD/token) the gap between the correct 0.02
    // multiplier and the retired 0.1 multiplier is only ~5.6e-8 USD at small token counts —
    // smaller than the cost header's 6-decimal-place (1e-6 USD) rounding floor, so a small-count
    // test cannot actually distinguish the two. At this scale the gap widens to ~5.6e-4 USD.
    wiremock_stubs::stub_openai_chat_with_cache(
        &mock,
        "deepseek-v4-flash",
        100_000,
        20_000,
    ...
    // deepseek-v4-flash: input=1.4e-7 USD/token, output=2.8e-7 USD/token, cache_read_multiplier=0.02
    // mock: 100,000 prompt (50,000 cached), 20,000 completion; Inclusive accounting →
    // non-cached input_tokens = 50,000
    // expected = 50,000 * 1.4e-7 * 0.02  (cached input at 2% rate)
    //          + 50,000 * 1.4e-7         (non-cached input)
    //          + 20,000 * 2.8e-7         (output)
    //          = 0.01274 USD
    // Tolerance: 5e-6, comfortably above 6-decimal-place header rounding noise (max 5e-7) and
    // comfortably below the ~5.6e-4 USD gap to the retired 0.1 multiplier (0.0133 USD) — see the
    // stub call above for why small token counts can't make this assertion discriminative.
    let expected_usd = 50_000.0 * 1.4e-7 * 0.02 + 50_000.0 * 1.4e-7 + 20_000.0 * 2.8e-7;
    assert!(
        (actual_usd - expected_usd).abs() < 5e-6,
        "deepseek-v4-flash cost must reflect cache_read_multiplier 0.02: \
         expected {expected_usd:.9} USD, got {actual_usd:.9}"
    );
}

/// Newly-imported Chinese-vendor entry end to end: glm-5's cache_read_multiplier of 0.2 is
/// applied rather than defaulted.
///
/// This pins a specific regression. An earlier revision of the snapshot omitted
/// `cache_read_multiplier` on glm-5 entirely; `BundledCostCalculator` reads it as
/// `unwrap_or(1.0)`, so cached input tokens were silently billed at the full input rate — a 5x
/// over-charge that no structural check on the asset could detect, because an absent optional
/// field is perfectly valid JSON.
#[tokio::test]
async fn compat_glm5_applies_cache_read_multiplier() {
    let pg = PgContainer::start().await.expect("pg must start");
    let redis = RedisContainer::start().await.expect("redis must start");

    let mock = wiremock::MockServer::start().await;
    wiremock_stubs::stub_openai_chat_with_cache(&mock, "glm-5", 100_000, 20_000, 50_000).await;
    ...
    // glm-5: input=1e-6 USD/token, output=3.2e-6 USD/token, cache_read_multiplier=0.2
    // mock: 100,000 prompt (50,000 cached), 20,000 completion; Inclusive accounting →
    // non-cached input_tokens = 50,000
    // expected = 50,000 * 1e-6 * 0.2  (cached input at 20% rate)   = 0.010
    //          + 50,000 * 1e-6        (non-cached input)           = 0.050
    //          + 20,000 * 3.2e-6      (output)                     = 0.064
    //          = 0.124 USD
    // A defaulted 1.0 multiplier would yield 0.164 USD, so the 5e-6 tolerance separates the two
    // by four orders of magnitude.
    let expected_usd = 50_000.0 * 1e-6 * 0.2 + 50_000.0 * 1e-6 + 20_000.0 * 3.2e-6;
    assert!(
        (actual_usd - expected_usd).abs() < 5e-6,
        "glm-5 cost must reflect cache_read_multiplier 0.2: \
         expected {expected_usd:.9} USD, got {actual_usd:.9}"
    );
}
