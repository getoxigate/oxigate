...
//!
//! Emits `oxigate_requests_total{method, status, provider, endpoint}` and
//! `oxigate_request_duration_seconds{provider, endpoint}` for every v1 LLM request.
//!
//! The `provider` label value is read from a [`ProviderLabel`] extension injected
//! into the response by the route handler. If absent (e.g. for non-chat routes),
//! it falls back to `"unknown"`. The `endpoint` label is derived from the request URI path.
...
use crate::observability::metrics::{
    ENDPOINT_CHAT, ENDPOINT_EMBEDDINGS, ENDPOINT_OTHER, REQUEST_DURATION_SECONDS, REQUESTS_TOTAL,
};
    ...
    E: Send + 'static,
{
    ...
    fn call(&mut self, req: Request) -> Self::Future {
        ...
        let endpoint = match req.uri().path() {
            p if p.ends_with("/chat/completions") => ENDPOINT_CHAT,
            p if p.ends_with("/embeddings") => ENDPOINT_EMBEDDINGS,
            _ => ENDPOINT_OTHER,
        };
        ...
        Box::pin(async move {
            ...
            .increment(1);
            metrics::histogram!(REQUEST_DURATION_SECONDS, "provider" => provider, "endpoint" => endpoint)
                .record(elapsed);
