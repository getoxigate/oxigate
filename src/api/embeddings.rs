...
fn record_embedding_metrics(provider: &str, latency_secs: f64, status: EmbeddingStatus) {
    metrics::counter!(
        EMBEDDINGS_TOTAL,
    ...
    .increment(1);
    metrics::histogram!(EMBEDDINGS_DURATION_SECONDS, "provider" => provider.to_owned())
        .record(latency_secs);
}
...
pub async fn embeddings(
    ...
    match embed_result {
        Err(e) => {
            record_embedding_metrics(&provider_name, latency_secs, EmbeddingStatus::Error);
        ...
        Ok(response) => {
            ...
            let mut resp = if let Some(ref usage) = response.usage {
                ...
                record_embedding_metrics(&provider_name, latency_secs, EmbeddingStatus::Success);
                metrics::counter!(EMBEDDINGS_VECTORS_TOTAL, "provider" => provider_name.clone())
