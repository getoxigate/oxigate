...
Third-party material redistributed in this repository carries its own notices — see
[`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md). The bundled model pricing data is embedded
into the binary at build time, so that file must accompany binary redistributions too.
